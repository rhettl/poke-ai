/**
 * Shared algorithms used by both StrongBattleAI and EmeraldPlusAI.
...
/**
 * Returns the average type effectiveness of all damaging moves an attacker has
 * against a defender, ignoring disabled moves. Falls back to 1.0 if no
 * damaging moves are available.
 *
 * Prefer this over bestTypeEffectiveness when evaluating switch quality —
 * it avoids inflating scores for mons that have one coverage move alongside
 * three irrelevant ones.
 *
 * @param {import('../types.ts').Pokemon} attacker
 * @param {import('../types.ts').Pokemon} defender
 * @returns {number}
 */
export function averageTypeEffectiveness(attacker, defender) {
  const moves = attacker.moves.filter(m => m.power > 0 && !m.disabled);
  if (moves.length === 0) return 1.0;
  const sum = moves.reduce((s, m) => s + typeEffectiveness(m.type, defender.types), 0);
